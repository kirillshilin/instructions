...
Every `<name>.fn.ts` must have a `<name>.fn.spec.ts`. The test validates **the wiring** — that the function calls the right service method with the right arguments. Business logic is tested in the service spec.

See [shai-typescript-testing](../../shai-typescript/instructions/shai-typescript-testing.instructions.md) for base structure (AAA, `describe`/`it` naming, `jest.Mocked<T>`).
